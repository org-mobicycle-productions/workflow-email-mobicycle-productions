...
export async function handleTriageSimple(request: Request, env: Env): Promise<Response> {
  const all = await triageNamespace(env.FILTERED_DATA_HEADERS, 'FILTERED_DATA_HEADERS');
  const matched = all.filter(d => d.level === 'SIMPLE');
  ...
  if (request.method === 'POST') {
    for (const d of matched) {
      ...
      email.status = 'queued_simple';
      email.triageLevel = 'SIMPLE';
