...
export async function handleTriageComplexityHigh(request: Request, env: Env): Promise<Response> {
  const all = await triageNamespace(env.FILTERED_DATA_HEADERS, 'FILTERED_DATA_HEADERS');
  const matched = all.filter(d => d.level === 'HIGH_COMPLEX');
  ...
  if (request.method === 'POST') {
    for (const d of matched) {
      ...
      email.status = 'queued_high_complex';
      email.triageLevel = 'HIGH_COMPLEX';
