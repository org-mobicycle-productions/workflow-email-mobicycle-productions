...
import classificationRulesJson from '../../classification-rules.json';
...
// Enhanced email matching: checks sender, recipient, AND subject
function matchEmailAgainstRules(
	email: { from: string; to: string; subject: string },
	rules: any[]
): { matched: boolean; namespaces: string[]; categories: string[] } {
	...
	for (const rule of rules) {
		...
		// Check fromIncludes
		if (conditions.fromIncludes) {
			for (const pattern of conditions.fromIncludes) {
		...
		// Check toIncludes
		if (!hits && conditions.toIncludes) {
			for (const pattern of conditions.toIncludes) {
				if (toLower.includes(pattern.toLowerCase())) {
					hits = true;
					break;
				}
			}
		...
		// Check subjectIncludes
		if (!hits && conditions.subjectIncludes) {
			for (const pattern of conditions.subjectIncludes) {
		...
		if (hits) {
			matchedNamespaces.push(rule.namespace);
			matchedCategories.push(rule.namespace);
		}
	}

	return {
		matched: matchedNamespaces.length > 0,
		namespaces: [...new Set(matchedNamespaces)],
		categories: [...new Set(matchedCategories)]
	};
}

// Rose's email addresses - filter these out from inbound processing
...
async function executeEmailFetchingJob(env: MobicycleOUEnv): Promise<void> {
	...
	try {
		...
		const data = await imapResponse.json() as any;
		const emails = data.emails || [];
		console.log(`[CRON:FETCHING] Fetched ${emails.length} emails from All Mail`);

		// Step 2: Filter out emails FROM Rose (pipeline spec step 3)
		const inbound = emails.filter((e: any) => {
			const fromLower = (e.from || '').toLowerCase();
			return !ROSE_ADDRESSES.some(addr => fromLower.includes(addr));
		});
		console.log(`[CRON:FETCHING] ${inbound.length} inbound (filtered ${emails.length - inbound.length} sent by Rose)`);

		// Step 3: Save ALL inbound to RAW_DATA_HEADERS
		for (const email of inbound) {
			...
			const emailValue = formatEmailValue(
				...
				email.messageId || '', email.body || '', 'RAW'
			);
			await env.RAW_DATA_HEADERS.put(emailKey, JSON.stringify(emailValue));
		}

		// Step 4: Load REAL classification rules and match
		const rules = classificationRulesJson.rules || [];
		console.log(`[CRON:FETCHING] Loaded ${rules.length} classification rules`);

		let filteredCount = 0;
		const routeStats: Record<string, number> = {};

		for (const email of inbound) {
			const result = matchEmailAgainstRules(
			...
			const emailValue = formatEmailValue(
				email.from || '', email.to || '', email.subject || '',
				email.date || new Date().toISOString(),
				email.messageId || '', email.body || '',
				result.categories.join(',')
			);

			// Store in FILTERED_DATA_HEADERS
			await env.FILTERED_DATA_HEADERS.put(emailKey, JSON.stringify(emailValue));

			// Route to ALL matched KV namespaces (multi-namespace routing)
			for (const nsName of result.namespaces) {
				const namespace = (env as any)[nsName];
				if (namespace) {
					await namespace.put(emailKey, JSON.stringify(emailValue));
					routeStats[nsName] = (routeStats[nsName] || 0) + 1;
				} else {
					console.warn(`[CRON:FETCHING] KV binding not found: ${nsName}`);
				}
			}
		}

		// Step 5: Store pipeline run status
		...
		console.log(`[CRON:FETCHING] Done: ${emails.length} fetched, ${inbound.length} inbound, ${filteredCount} classified`);
