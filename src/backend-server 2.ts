...
/**
 * ProtonMail Bridge Backend Server for rose@mobicycle.productions
...
// ProtonMail Bridge IMAP settings for rose@mobicycle.productions
const IMAP_CONFIG = {
	host: '127.0.0.1',
	port: 1143,
	secure: false,
	auth: {
		user: 'rose@mobicycle.productions',
	...
	async fetch(request: Request): Promise<Response> {
		...
		try {
			...
			if (path === '/api/kpi/total-accounts') {
				return new Response(JSON.stringify({ count: 1, accounts: ['rose@mobicycle.productions'] }), {
					headers: { 'Content-Type': 'application/json', ...corsHeaders }
				});
			}
			...
			if (path === '/account-info') {
				return new Response(JSON.stringify({
					email: 'rose@mobicycle.productions',
			...
			if (path.startsWith('/label-sent/')) {
				...
				const email = url.searchParams.get('email') || 'rose@mobicycle.productions';
				return new Response(JSON.stringify({ 
					label, 
					email, 
			...
			if (path === '/') {
				return new Response(JSON.stringify({
					...
					account: 'rose@mobicycle.productions',
					version: '1.0.0',
					endpointCount: 56,
					endpoints: {
						health: 'GET /health',
						...
						// Email Queries
						sentByEE: 'GET /sent-by-rose@mobicycle.productions',
						...
						sentToEE: 'GET /sent-to-rose@mobicycle.productions',
						...
						// No Spam variants
						sentToEENoSpam: 'GET /sent-to-rose@mobicycle.productions-no-spam',
					...
					tunnel: 'https://mail.mobicycle.productions',
				}), {
					headers: { 'Content-Type': 'application/json', ...corsHeaders },
				});
			}
...
console.log(`🔗 Tunnel: https://mail.mobicycle.productions`);
console.log('\nEndpoints:');
